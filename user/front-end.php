...
class cgss_do_table {
	...
	public function server_seo() {
			...
			$tbl_data_type = array();
			$input_data = array(
								array( __( 'SSL security', 'cgss' ), 'SSLChk' ),
								array( __( 'WWW Resolve', 'cgss' ), 'WWWChk' ),
								array( __( 'IP forwarding', 'cgss' ) . '<span id="IpVal"></span>', 'IPChk' ),
								array( __( 'Gzip Compression', 'cgss' ), 'GzipChk' ),
								array( __( 'Cache', 'cgss' ), 'CacheChk' ),
								array( __( 'If modified since', 'cgss' ), 'IfModChk' ),
								array( __( 'Response Time', 'cgss' ) . '<span id="ResVal"></span> ' . __( 'miliseconds', 'cgss' ), 'ResTChk' ),
							);
			foreach ( $input_data as $val ) {
				$tbl_data_type[] = array(
										array(
...
class cgss_elements {
	...
	public function action() {
		$arr = array(
					...
					array( 'VportAct', __( '<code>< meta name="viewport"</code> tag should be present for scaling webpage with device size.', 'cgss' ), '', 'smartphone' ),
					array( 'CssMediaAct', __( '<code>@media</code> query is not found. The webpage should be mobile ready.', 'cgss' ), '', 'smartphone' ),
					...
					array( 'CanoAct', __( 'Put a canonical link.', 'cgss' ), '', 'randomize' ),
					array( 'IfModAct', __( 'Enable If modified since header.', 'cgss' ), '', 'randomize' ),
					array( 'RoboAct', __( '<code>< meta name="robots"</code> should not have <code>noindex</code> as value.', 'cgss' ), '', 'randomize' ),
					array( 'IpAct', __( 'Put an IP address and forward to page url', 'cgss' ), '', 'randomize' ),
					array( 'SpeedAct', __( 'Header response time is too long. Improve website server.', 'cgss' ), '', 'clock' ),
					array( 'GzipAct', __( 'Enable gZip compression.', 'cgss' ), '', 'clock' ),
					array( 'CacheAct', __( 'Enable Browser Caching.', 'cgss' ), '', 'clock' ),
	...
	public function content() {
		...
		return '<div id="keyDis"></div>' .
				...
				'<div class="row">
					<div class="col-3">' .
						$this->slide( 'words', '<span id="WordsSwitch"></span> ' . __( 'Number of words', 'cgss' ), 'WordsNum', __( 'Check if url is showing same webpage for url with and without "www." prefix." prefix.', 'cgss' ), false ) .
						...
						$this->slide( 'thrcy', '<span id="ThrchySwitch"></span> ' . __( 'Text hiarerchy', 'cgss' ), 'Thrchy', __( 'A bulk of text is not useful, rather text conatent with proper heading tags are needed.', 'cgss' ), false ) .
					'</div>
					<div class="col-3">' .
	...
	public function design() {
		return '<div class="img-list border-top">
				...
				'<div class="row">
					...
					'</div>
					<div class="col-3">' .
						$this->slide( 'media', '<span id="MdQrSwitch"></span> ' . __( 'Media Queries', 'cgss' ), 'MdQrVal', __( 'For responsive web design, you need to have <code>@media</code> elements inside .css files.', 'cgss' ), 'https://developers.google.com/web/fundamentals/layouts/rwd-fundamentals/use-media-queries?hl=en' ) .
						$this->slide( 'vport', __( 'Viewport Tag', 'cgss' ), 'VportOnOff', __( 'On <code>head</code> area of webpage, there is a tag which helps in scaling the dimeensions according to display size.', 'cgss' ), 'https://developers.google.com/speed/docs/insights/ConfigureViewport' ) .
					'</div>
				</div>';
	}
	...
	public function crawl() {
		return '<div class="show-url">
				...
				<div class="row">
					<div class="col-3">' .
						$this->slide( 'www', __( 'www Resolve', 'cgss' ), 'WwwOnOff', __( 'Check if url is showing same webpage for url with and without "www." prefix." prefix.', 'cgss' ), false ) .
					...
					'</div>
					<div class="col-3">' .
						$this->slide( 'cano', __( 'Canonical Url', 'cgss' ), 'CanoOnOff', __( 'Let Google know, same content is available through multiple URL structures or via syndication.', 'cgss' ), 'https://support.google.com/webmasters/answer/139066?hl=en' ) .
						$this->slide( 'robo', '<span id="RoboChk"></span> ' . __( 'Meta Robot', 'cgss' ), 'RoboVal', __( 'Visibility in search result stops on use of "noindex" value in robots meta tag.', 'cgss' ), 'https://support.google.com/webmasters/answer/93710?hl=en' ) .
					'</div>
				</div>' .
				$this->slide( 'ip', '<span id="IpSwitch"></span> ' . __( 'IP Forwarded', 'cgss' ), 'IpAddr', __( 'Check if server IP address (internet protocol) is forwarding to the same url or not.', 'cgss' ), 'https://support.google.com/webmasters/answer/139066?hl=en' );
	}
	...
	public function report_time() {
		return '<ul>
				...
				<div class="row">
					<div class="col-3">' .
						$this->slide( 'gzip', __( 'Gzip Compression', 'cgss' ), 'GzipOnOff', __( 'Deliver page content in a compressed .gzip file to reduce bandwidth and loading time.', 'cgss' ), 'https://developers.google.com/speed/docs/insights/EnableCompression' ) .
						$this->slide( 'cache', __( 'Browser Caching', 'cgss' ), 'CacheOnOff', __( 'Store page assets in browser of user for specific time, to load it faster next time.', 'cgss' ), 'https://developers.google.com/speed/docs/insights/LeverageBrowserCaching' ) .
					'</div>
					<div class="col-3">
